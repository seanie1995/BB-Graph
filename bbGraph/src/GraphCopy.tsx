...
function Graph() {
  ...
  const x0 = 25;
  const xAxisLength = SVG_WIDTH - x0; // The width needs to be subtracted by the starting point so that the graph itself becomes more 'centered' and doesn't hug the left wall of the graph. Multiplied by 2 in order to make it longer
  ...
  const barPlotWidth = (xAxisLength / data.length) + 2.8 ; // Evenly distributes space for each month.
      ...
      {data.map(([month, dataY], index) => {
        ...
        return (
          <g key={index}>
            ...
            <text className="monthLabel" x={x} y={xAxisY + 18} textAnchor="middle">
              {month}
            </text>
